    ...
    **kwargs
    ...
    if optimize:
        ...
        for i, extracted_data in enumerate(extracted_data_list):
            if extracted_data.get("markdown"):
                ...
                logger.info(f"优化完成 ({i+1}/{len(extracted_data_list)}): {extracted_data.get('metadata', {}).get('url', '')}")
            else:
    ...
    if output_dir:
        ...
        for i, result in enumerate(results):
            ...
            filename = f"url_{i+1}.md"
            
            with open(os.path.join(output_dir, filename), 'w', encoding='utf-8') as f:
                f.write(result.get("markdown", ""))
            
            # 保存元数据
            with open(os.path.join(output_dir, f"url_{i+1}.json"), 'w', encoding='utf-8') as f:
...
def main():
    """命令行入口函数"""
    parser = argparse.ArgumentParser(description="Web Benchmark Agent - HTML到Markdown转换工具")
    parser.add_argument("--url", help="要处理的URL")
    parser.add_argument("--urls-file", help="包含URL列表的文件路径")
    parser.add_argument("--extractor", choices=["firecrawl", "jina"], 
                        default="firecrawl", help="使用的提取器")
